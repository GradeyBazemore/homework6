 ...
 *     parameters:
 *       - name: lastName
 *         description: The last name of the student to search for.
 *         in: query
 *         required: true
 *         schema:
 *           type: string
 ...
 *       404:
 *         description: Error. No matching students found.
 */

app.get('/students/search/lastname', function (req, res) {
  const searchLastName = req.query.lastName; // Get the search parameter from the query string
  app.get('/students/search/lastname', function (req, res) {
    const searchLastName = req.query.lastName;
    
    // Assuming you have an array of student objects
    const matchingStudents = students.filter(student => student.last_name === searchLastName);
